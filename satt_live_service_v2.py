...
class PolymarketExecutionClient:
    def __init__(self):
        ...
        # 0 = EOA (MetaMask), 1 = Proxy (Email/Google). По умолчанию 1 для безопасности
    ...
    def initialize(self):
        ...
        # Шаг 1: Создаем временный клиент для генерации L2 ключей
        ...
        # Шаг 2: Инициализируем полноценный торговый клиент с креденшелами
    ...
    def place_market_buy(self, token_id, usdc_amount, tick_size):
        """Размещает рыночный ордер на покупку (указывается сумма в долларах)"""
        try:
    ...
    def place_limit_sell(self, token_id, shares, price, tick_size="0.01"):
        """Размещает лимитный ордер на продажу (указывается кол-во акций и цена)"""
        try:
            # В V2 обязательно нужно передавать tick_size в options
...
def main():
    ...
    val = sync_snapshot['portfolio_value']
